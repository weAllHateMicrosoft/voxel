...
class WindowHud {
    ...
    void renderHelpScreen(float screenW, float screenH) {
        ...
        helpRow("[']",  "'Deprivation Domain' — Water God Stance (testing). Tap ['] to lock yourself in place and unleash an absolute counter-attack domain. A golden HDR hemisphere of radius " + (int)GameConfig.depRadius + " blocks appears around you. Any enemy or projectile that moves inside it is instantly killed and connected to you by a glowing gold 'thread' that lingers for 2 seconds. The thread web builds as more enemies fall. The world inside the domain desaturates and tints gold; outside turns cool and distant. The domain lasts " + (int)GameConfig.depDuration + " s. Press ['] again to exit early. Tune all values in GameConfig (dep*). Inspired by Mushoku Tensei's Deprivation Field + Vergil's Dimensional Slash.");
